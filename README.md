...
- `turn/steer`：不可直接输入的 multi-agent 子代理失败时，通知仍在运行的父 turn 检查并重新
  派发子任务。
- `turn/start`：普通 thread 满足恢复条件后开始新的继续 turn；子代理的父 thread 空闲时，
  也用它启动父线程恢复。
...
thread 的 `turn/start`，并发送“继续”消息，因此不会创建新的 Codex thread。启用探针时，
恢复定时器会立即进入门禁等待，不会在探针放行前后额外叠加
`CODEX_WATCHDOG_DELAYS_MS`。探针关闭时，goal 模式保持原有恢复行为；普通模式不会启用
这条自动续跑路径，但下面的精确 capacity 错误不受此限制。

Codex app-server 不允许对 multi-agent v2 子代理直接调用 `turn/start`。watchdog 会通过
`thread/started` 记录 `parentThreadId` 和 `canAcceptDirectInput`；子代理需要恢复时，向最近
可直接输入的父线程发送恢复请求。父线程仍有活动 turn 时使用 `turn/steer`，空闲时使用
`turn/start`，让父线程检查失败子任务并在需要时重新派发。
...
恢复退避；当前 turn 确认失败或 goal 进入 `blocked` 后会立即恢复，不会增加普通故障的
退避计数，并且不会伪装成 429。
