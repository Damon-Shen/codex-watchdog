...
**Goal:** Replace the root README with an accurate, user-first guide for the current Codex recovery core, relay plugin runtime, and bundled ai.input.im plugin while preserving original authorship.

**Architecture:** Keep the root README as the concise entry point: quick installation and ai.input.im setup first, recovery and plugin architecture second, with detailed contracts delegated to existing docs. Do not change runtime code, plugin code, configuration examples, or the license.

**Tech Stack:** GitHub-flavored Markdown, Node.js 22+, npm, native repository verification scripts.
...
- [ ] **Step 1: Replace the complete README content**

Replace `README.md` with the following exact content:
...
- [ ] **Step 2: Inspect the README-only diff**
...
Expected: `README.md` is the only uncommitted file; no runtime, plugin, configuration, or license file changes.

### Task 2: Verify documentation accuracy and publish the rewrite

**Files:**
- Verify: `README.md`
- Verify: `LICENSE`
- Verify: `package.json`
- Verify: `docs/plugins/README.md`
- Verify: `plugins/aiinput.mjs`
- Verify: `plugins/aiinput.example.json`

- [ ] **Step 1: Verify every relative Markdown link resolves**

Run:

```bash
node --input-type=module -e 'import { existsSync } from "node:fs"; import path from "node:path"; const text = await (await import("node:fs/promises")).readFile("README.md", "utf8"); const links = [...text.matchAll(/\]\(([^)]+)\)/g)].map((match) => match[1]).filter((target) => !/^(?:https?:|#)/.test(target)); const missing = links.filter((target) => !existsSync(path.resolve(target.split("#")[0]))); if (missing.length) { console.error(missing); process.exit(1); } console.log(`README relative links OK (${links.length})`);'
```

Expected: exit 0 and `README relative links OK`.

- [ ] **Step 2: Verify preserved attribution and prohibited behavior wording**

Run:

```bash
rg -n '原作者.*flowing-water1|Copyright \(c\) 2026 flowing-water1|Linux\.do|MIT License' README.md
rg -n '不维护连续 429|没有.*连续 429.*熔断' README.md
```

Expected: all preserved attribution items and the explicit no-counter/no-circuit-breaker statement are present.

- [ ] **Step 3: Verify referenced commands and files exist**

Run:

```bash
node -e 'const p = require("./package.json"); for (const name of ["test", "check", "test:cli", "test:live"]) { if (!p.scripts[name]) throw new Error(`missing script ${name}`); }'
test -f bin/codex-watchdog.mjs
test -f bin/codex-watchdog.ps1
test -f bin/codex-watchdog.cmd
test -f plugins/aiinput.mjs
test -f plugins/aiinput.example.json
...
Expected: 126 tests pass; syntax, installed CLI, live app-server smoke, and whitespace checks exit 0.

- [ ] **Step 5: Confirm the final scope**
...
Expected: only `README.md` is modified, apart from the already committed design and plan documents.

- [ ] **Step 6: Commit the README rewrite**

```bash
git add README.md
git commit -m "docs: rewrite README for plugin mode"
```

- [ ] **Step 7: Push the current branch to the writable fork main**
...
Expected: a normal fast-forward update of `Damon-Shen/codex-watchdog` `main`; do not force-push.
